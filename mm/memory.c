...
#define PAGE_TABLE_ADDR     0x40040000

/*
 * 设置页表
 */
void create_page_table(int viraddr_begin,int viraddr_end,int phyaddr,char flag)
{
    //printf("\r\ntext_start:0x%x bss_end:0x%x\r\n",text_start,bss_end);
    unsigned long virtuladdr, physicaladdr;
    unsigned long *mmu_tlb_base = (unsigned long *)PAGE_TABLE_ADDR; //SDRAM
     ...
     */
    virtuladdr = viraddr_begin;
    physicaladdr = phyaddr;
    while(virtuladdr <= viraddr_end)
    {
		//printf("virtuladdr:0x%x physicaladdr:0x%x\r\n",virtuladdr,physicaladdr);
        if(!flag)
...
void clean_page_table(void)
{
    int i;

    unsigned long *mmu_tlb_base = (unsigned long *)PAGE_TABLE_ADDR;
